...
function ProductDescription({navigation}) {
  ...
  return (
    <ScrollView style={{backgroundColor:"white"}}>
     ...
     <TouchableOpacity style={{alignItems:"flex-end", paddingRight:10}} onPress={()=>setwishlist(!wishlist)}>
     {wishlist? <AntDesign name="heart" size={24} color="red" />:<AntDesign name="hearto" size={24} color="#EBEDF0" />}
      ...
      <View style={styles.details}>
        ...
        <Text style={styles.rating}>{`${item?.hidden_stars} (${item?.ratings})`}</Text>
        
        </View>
        <Text style={{fontSize: 12,
    color: 'green',paddingBottom:5}}>{item?.delivery_type}</Text>
        <View style={{backgroundColor:"#E6F0FF", padding:10}}>
        <Text style={{fontSize: 12,
    color: 'green',
    
    marginRight:10}}>Special Price</Text>
    ...
    marginTop: 4,
   fontWeight:"bold",
    marginRight:10}}>{`${item?.discount}% off`}</Text>
        <Text style={{fontSize: 18,
    color: 'gray',
    marginTop: 4,
    fontWeight:"bold",
    textDecorationLine: 'line-through',
    marginRight:10}}>₹{item?.old_price}</Text>
